...
struct bld__task
{
	...
	os_utf8 command[bld__MAX_COMMAND];
...
os_result bld__gather_tasks(struct json__object *obj, void *ctx)
{
	...
	if (json__match(&stack[2], (os_utf8*)"tasks"))
	{
		if (obj->depth > 3 && json__match(&stack[3], (os_utf8*)""))
		{
			...
			else if (obj->depth > 4 && json__match(&stack[4], (os_utf8*)"linux"))
			{
				if (json__match(&stack[5], (os_utf8*)"command"))
				{
					json__copy_string(obj, obj->value_start, obj->value_end, sizeof(t->command), t->command);
...
struct bld__vardata {
	os_utf8 *name;
	os_utf8 value[bld__MAX_COMMAND];
};

os_result bld__get_var(struct json__object *obj, void *ctx)
{
	struct json__object *stack = obj->tree->stack;
	os_utf8 *s;
	os_intn i;
	struct bld__vardata *d = ctx;
	if (json__match(obj, d->name)) {
		json__copy_string(obj, obj->value_start, obj->value_end, 
				bld__MAX_COMMAND, d->value);
	}
	return 0;
}
	
os_result bld__expand(os_utf8 *cmd, struct json__object *settings)
{
	os_utf8 *p;
	int n = 32;
	int i, j, k, l;
	...
	static os_utf8 tmp[bld__MAX_COMMAND + 8];
	i = 0;
	j = 0;
	...
	while (p && n) {
		...
		l = strlen(s);
		k = strlen(tmp + i + j);
		if (i + l + k >= bld__MAX_COMMAND) {
			exit(-1);
		}
		memmove(tmp + i + l , tmp + i + j, k + 1);
		memmove(tmp + i, s, l);
		p = (os_utf8*)strstr((char*)tmp, "${config:");
		n--;
	}
		...
		struct json__object *settings)
{
	...
	bld__expand(t->command, settings);
	   ...
	   struct json__object *settings)
{
	...
	return bld__exec(t, workspace, settings);
}
		...
		struct json__object *settings)
{
	...
	if (t)
	{
		return bld__depends(root, t, 0, workspace, settings);
	}
	else
	{
...
int main(int argc, char *argv[])
{
	os_utf8 *json;
	os_size size;
	os_utf8 *settings_json;
	os_size settings_size;
	struct json__tree tree;
	struct json__tree settings_tree;
	struct json__object *obj;
	struct json__object *settings_obj;
	os_utf8 path[1024];
	struct bld__task *t;
	...
	snprintf((char *)path, sizeof(path) - 1, 
			"%s/.vscode/settings.json", argv[1]);
	settings_json = json__load_file(path, &settings_size);
	if (!settings_json)
	{
		exit(-1);
	}

	settings_obj = json__parse(settings_json, settings_size, &settings_tree);
	...
	if (settings_obj->type == json__undefined)
	{
		printf("cannot find root object\n");
	}
	...
	*/
	...
	bld__make(t, (os_utf8 *)argv[1], (os_utf8 *)argv[2], settings_obj);
	bld__free(t);
	free(json);
	free(settings_json);
	return 0;
